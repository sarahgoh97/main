...
In a prison, it is necessary to have cells, where prisoners stay in. It is important to keep track of the prisoners in the cell
and whether the cell is full.

This cell system is most clearly represented using a 2-D array. Hence, this was used as the basis of the map system of the prison.
When implemented, it will take up the right side area of the application (but this has not yet been implemented). Below is
an idea of what the UI may look like by the end of the project. The current implementation is still shown in the box below the user input.
This basic map shows the basic implementation of the cell system and helps keep track of what is happening in the cells.

image::currentMap.PNG[width="400"]

image::futureMap.PNG[width="400"]

With this map system of cells, the cells can be viewed. Not only that, prisoners can also be added into them and removed from them.
...
represented by the square brackets) or in individual cells.

To ensure that the number of prisoners in each cell does not exceed its limits, exceptions will be put in place to prevent such occurences.
Also, exceptions that prevent users from adding prisoners into cells that do not exist (ie. there is no such cell in the prison).
...
may grow or shrink in the future. In this scenario, we will strive only to hit our non-functional requirements.
...
This then adds a prisoner into the specified cell.
[source,java]
----
//in Cell.java
...
Suppose that the user has access to the functions to add prisoners into prison cells.

The cell map will have all the cells and its current occupants.

The user executes a new `addCommand`. If the command is parsed successfully, and if the person added is a prisoner,
the display result will prompt the user with a question to continue to add the prisoner to a cell with a map shown below it.

Once the user uses the addcell command for the new prisoner, the prisoner is then added into the cell and the cell map system is updated.
...
For future implementations, there will also be a command to remove prisoners, when they are being transferred to another cell,
or if they can leave prison and go back home to their loved ones. This will be done in a similar manner to addcell.

In much further implementations, guards/prison workers can also be assigned to cells, not as prisoners but as according to their job descriptions.
...
** Cons: Harder to do more functions that the Java API has provided. Does not fit in with the other models of Person and Tag with their UniquePersonList and UniqueTagList
...
* **Alternative 1 (current choice):** Create a new Command to show the map.
...
* **Alternative 2:** Keep the map always on display.
** Pros: Easier to see when adding prisoners to cells.
** Cons: Risk security as access to map is not limited. Even if it is, anyone walking by can see the information easily.


===== Aspect: Implementation of AddCellCommand (addcell)

* **Alternative 1 (current choice):** Prisoners are added into cells in a cellMap.
** Pros: Cells are always there and can hold prisoners. Since these groups are still Persons with a guard role or prisoner role, not all persons may have a cell.
** Cons: More difficult to see the individual's cell. (have not implemented updating addresses)
...
** Cons: Harder to manage a system of cells. Easy to accidentally overcrowd a cell.
...
===== Aspect: Implementation of RemoveCellCommand (removecell)

* **Alternative 1 (current choice):** Can choose to remove prisoner from a cell and will automatically remove from prison when prisoner is deleted.
** Pros: Allows users to transfer prisoners to other cells (with addcell as well). Less administration required of user.
** Cons: Events-driven system to call RemoveCellCommand will be more complicated to coordinate
* **Alternative 2:** Choose to empty a prison cell.
** Pros: More efficient in removing prisoners, instead of 1 by 1.
** Cons: Need to handle more than one prisoner that has been released or broken out.
...
//tag::scheduling[]
=== Guard Scheduling Feature
==== Current Implementation

The Guard Scheduling feature can be accessed via entering the command `cal`.

It allows guards/wardens to interact with a Google Calendar account through the current PrisonBook GUI.

Calendar Commands that are supported are:
* add e/eventname t/timestamp
* remove [eventID]
* edit [eventID] e/eventname t/timestamp

The PrisonBook calendar commands will send HTTPS requests to the REST calendar API and make changes to the Google account calendar.
Upon confirmation of successful changes by the Google Calendar API, the change will be reflected next time any calendar commands are called.
...
===== Aspect: Implementation of editing calendar

* Alternative 1 (current choice): Using command line to type with prefixes for event names and their timings.
Pros: Ties in with users who prefer typing over mouse clicking.
Cons: Not as intuitive as selecting time slots to edit. Need more prefixes for users to learn.
* Alternative 2: Have buttons for users to click to edit.
 Pros: More intuitive as less things to type.
 Cons: No longer command line based.
...
* Sarah Goh Shi Ning (sarahgoh97)
  1. Major enhancement: +
  Creating a cell system/map of all the prison cells in the prison.
  ...
  2. Minor enhancement: +
  Allow user to change between bright and dark themes.
  This helps users to prevent people nearby from looking at the information.
  At the same time, it also has a bright theme that helps users use the app more easily.
